...
문서가 덧붙이는 근거가 더 설득력 있음. 타깃이 파일 목록을 들면 그 **목록에 순서가 생기는데 그 순서에 의미가 없음.** 의미 없는 순서 때문에 두 번째 diff 가 나고, 거기서 나는 충돌은 납득되지 않는 충돌이 됨. 원문 표현이 `harms readability, increases the likelihood of conflict, and makes conflict in that space feel unjustified since the order was meaningless` 임.
...
## 그래도 ObjectID 는 남아 있음

과장하기 쉬운 지점이라 따로 둠. 이름·경로 기반 참조로 갈아탄 것은 맞는데 **ID 가 사라진 것은 아님.** `ObjectID` 의 문서 주석이 범위를 정확히 그어 둠.
...
> "Some elements in Xcode can be referenced by external files. For example, Xcode targets can be referenced by other projects. These references are always ID based, and so some elements in a `project.xcproj` should always have IDs"

**최소화지 제거가 아니고**, 파일 바깥에서 들어오는 참조는 여전히 ID 기반임. `Project` 타입에도 `objectID`, `rootGroupDebugID`, `configurationListDebugID` 가 남아 있음.

이름 기반 참조에는 대가도 따라옴. `Project` 의 디코딩이 마지막에 `verifyReferenceIntegrity()` 를 부르고, 거기서 **타깃 이름 중복을 오류로 막음.**

```swift
// 검증 메시지
"Target names must be unique, but \(targetNames) \(isAre) used multiple times"
```

이름으로 참조를 만드는 포맷이니 당연한 제약인데, **이름이 겹치는 레거시 프로젝트를 변환하면 여기서 걸림.** 전환이 정보 손실 없는 작업이라는 것과 별개로, 전환 전에 타깃 이름부터 정리해야 하는 프로젝트가 있다는 뜻임.
...
`apple/xcode-project-format`. **2026-09-20 GitHub API 로 받은 값**임.

| 항목 | 값 |
|---|---|
| 라이선스 | **Apache-2.0** |
| 언어 | Swift |
| 별 / 포크 / 열린 이슈 | **384** / 12 / 10 |
| 생성 | 2026-09-15 |
| 최근 푸시 | 2026-09-16 |
...
**Xcode가 쓰는 바로 그 구현이 공개된 것**임. pbxproj 시절에는 도구마다 포맷을 리버스 엔지니어링해서 해석이 갈릴 수밖에 없었는데, 이제 **Xcode와 같은 해석을 고를 수 있는 선택지가 생김.**

**다만 갈릴 여지가 없어진 것은 아님.** 이 라이브러리를 쓰는 도구가 Xcode와 어긋나지 않는 것이지, 서드파티가 자체 파서를 쓸 수도 있고 버그가 있을 수도 있고 다른 스키마 버전을 대상으로 할 수도 있음. 아래 도입 판단에서 도구별 지원을 따로 확인하라고 적은 것이 그 때문임.
...
같은 문서가 AI 도구 사용에 대해 한 절을 따로 둠. 쓰는 것은 허용하되 제출자가 전적으로 책임지고 설명할 수 있어야 한다고 적고, 그 뒤에 이 문장이 붙음.
...
Apple 이 연 저장소의 기여 규칙에 이 문장이 들어간 것 자체가 볼거리임. 유지보수는 **전담 로테이션 없는 best effort**이고 응답에 며칠에서 몇 주가 걸릴 수 있다고 밝힘.
...
**2026-09-20 확인 기준이고 Xcode 27.2 beta(27B5019j) 시점임.** 별 수와 이슈 수는 며칠 단위로 낡으므로 갱신하지 말고 `gh api repos/apple/xcode-project-format`로 다시 받을 것. 다시 볼 때 확인할 것 둘. **27.2가 정식으로 나왔을 때 기본값과 호환성 문언이 그대로인지**, 그리고 **`0.1.0` 이후 태그가 붙으면서 스키마에 필드가 추가됐는지**. 후자는 `CONTRIBUTING.md`가 "expect this scope to grow over time"이라고 예고해 둔 부분임.
